...
#include "system.h"
/*
#ifndef JACKIE_MODIFY
#define JACKIE_MODIFY
#endif
*/
#include "list.h"
List ThreadList;
...
Thread::Thread(char* threadName)
{
    ...
    {
        DEBUG('t', "Already have %d thread running, can't create more!\n", ThreadSum);
        return;
    }
    name = threadName;
    stackTop = NULL;
...
{
//#ifdef JACKIE_MODIFY
    DEBUG('t', "Forking thread \"%s\"(uid:%d, tid:%d) with func = 0x%x, arg = %d\n",
      name, uid, tid, (int) func, (int*) arg);
/*#else    
...
//#ifdef JACKIE_MODIFY
    DEBUG('t', "Finishing thread \"%s\"(uid:%d, tid:%d)\n", getName(), uid, tid);
/*#else    
    DEBUG('t', "Finishing thread \"%s\"\n", getName());
#endif*/
    

    threadToBeDestroyed = currentThread; //lesson: bug!! may overwrite
...
//#ifdef JACKIE_MODIFY
    DEBUG('t', "Yielding thread \"%s\"(uid:%d, tid:%d)\n", getName(), uid, tid);
/*#else
...
//#ifdef JACKIE_MODIFY
    DEBUG('t', "Sleeping thread \"%s\"(uid:%d, tid:%d)\n", getName(), uid, tid);
/*#else
...
char*
strstatus(ThreadStatus st)
{
    switch (st)
    {
        case JUST_CREATED: return (char*) "JUST_CREATED";
...
{
    printf("TS below:\n");
    ThreadList.Mapcar((VoidFunctionPtr)ts);
    printf("TS finished.\n");
}
